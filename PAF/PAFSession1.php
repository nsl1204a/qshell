...
class PAFSession extends PAFObject
{
    ...
    function PAFSession ( $config )
    {
        ...
        if ( $config->isDefinedVariable("PATH_SESSION") )
            $this->pathSession= $config->getVariable("PATH_SESSION");
        else
            $this->pathSession= PS_DEFAULT_PATH_SESSION;

        // Fija el nombre de la cookie.
        if ( $config->isDefinedVariable("NAME_COOKIE") )
            $this->nameCookie= $config->getVariable("NAME_COOKIE");
        else
            $this->nameCookie= PS_DEFAULT_COOKIE_NAME;

        // Fija el dominio para la cookie.
        if ( $config->isDefinedVariable("DOMAIN_COOKIE") )
            $this->domainCookie= $config->getVariable("DOMAIN_COOKIE");
        else
            $this->domainCookie= PS_DEFAULT_COOKIE_DOMAIN;

        // Fija el Path para la cookie.
        if ( $config->isDefinedVariable("PATH_COOKIE") )
            $this->pathCookie= $config->getVariable("PATH_COOKIE");
        else
    ...
    function sessionStart()
    {
        ...
        session_save_path( $this->pathSession );
        ...
        session_start();
		$ficherito= $this->pathSession."/"."sess_".$this->id;
		echo "aing=> " . $ficherito."<br>";
	if ($this->changePermission) chmod($this->pathSession."/"."sess_".$this->id,octdec(777));
