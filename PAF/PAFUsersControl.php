...
class PAFUsersControl extends PAFObject
{
    /**
    ...
    /**
      ...
      */
    function PAFUsersControl(&$ds, $errorClass= null)
    {
...
*/
	...
	if ($ref = $_SERVER['HTTP_REFERER'])
	{
	   ...
	   if ($host != $host_ref) {
           $this->writeLog($aplicacion, $id, "REFERER_INVALIDO");
           return false;
           }
	}
...
//	if (!$cookie){
//        $this->writeLog($aplicacion, $id, "NO_COOKIE_CTRL");
//        return false;
//        }
	...
	if (!$s_machine) {
                $this->writeLog($aplicacion, $id, "IP_PRIVATE_OR_INVALID");
                return false;
        }
	...
	if ($filtro[$s_machine]) {
                $this->writeLog($aplicacion, $id, "IP_FILTERED");
                return false;
        }
	...
	$id = substr($id, 0, 255);
        ...
        if (PEAR::isError($this->result)) {
            $this->writeLog($aplicacion, $id, "ERROR_DB_SELECT");
            return $this->result;
        }
	...
	$timelimit = time() + $time * 60;
        ...
        if ( !$i_howmany ) {
            ...
            $this->result= $this->dataSource->runQuery ($query);
            if (PEAR::isError($this->result)) {
                $this->writeLog($aplicacion, $id, "ERROR_DB_INSERT");
                return $this->result;
            }
        }
        else { 
            ...
            if (strtotime($row["uc_ctrl_time"]) < time()) {
                ...
                $this->result= $this->dataSource->runQuery ($query);
                if (PEAR::isError($this->result)) {
                    $this->writeLog($aplicacion, $id, "ERROR_DB_UPDATE_TIME");
                    return $this->result;
                }

            }
            else {
                $this->writeLog($aplicacion, $id, "CTRL_TIME_LT_TIME");
                return false;
            }
        }    
    ...
    /**
      * Escribe un log en un fichero en caso de que se rechaze el comentario, con distintos casos para lo que 
      * se dan distintos mensajes de error.
      *
      * @access public
      * @param String $the_case en el que el comentario ha sido rechazado
      */
     function writeLog($aplicacion, $id, $the_case){
        
        $case_log = $the_case;
	$fp_failed = @fopen("/SESIONES/user_control/".date("Ymd")."ucontrol_failed.log", "a");
	if ($fp_failed)
	{
	   $vars = str_replace("\n", "", print_r($_POST, true));
	   //$vars = str_replace("\n", "", print_r($_COOKIE, true));
	   fwrite($fp_failed, "\"$case_log, \"$id\", \"$aplicacion\",  \"$vars\", \"\n");
	   fclose($fp_failed);
	}
