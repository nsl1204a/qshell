...
class PAFCacheOutputTest
{
	...
	function _generateStaticObject () {
        ...
        if (PEAR::isError($fileCacheName)) return PEAR::raiseError("ERROR: PAFCacheOutputTest. El KeyName es un PEAR::Error ==> ".$fileCacheName->getMessage(), 0, PEAR_ERROR_TRIGGER);
		...
		if (!@stat($fullpath))
		{
			...
			{
				PEAR::raiseError("ERROR: PAFCacheOutputTest. No pudo escribir serializado en $fullpath", 0, PEAR_ERROR_TRIGGER);
				if ($tracecache) echo "<pre> ERROR: PAFCacheOutputTest. No se pudo escribir serializado en $fullpath</pre>\n";
			}
		}
	}
