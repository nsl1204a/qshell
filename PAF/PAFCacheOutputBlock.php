...
require_once "PAF/PAFCacheBlock.php";
require_once "PAF/PAFHttpEnv.php";
require_once "PAF/PAFLog.php";


define ("PREFIX_NEWS_FILE_CACHE", "TES");
define ("DIR_ELPAIS_PATH_OBJ", "/SESIONES/appworx/encurso/elpais4");
...
class PAFCacheOutput
{
	...
	function PAFCacheOutput () {
		...
		$this->_cache = new PAFCacheBlock (basename($pathCache), dirname($pathCache));
		if ($_GET['log']=='cinco')
	...
	function _generateStaticObject () {
		...
		$fullpathSGE= DIR_ELPAIS_PATH_OBJ."/".get_class($this)."#".$this->_getKeyName().".ser";
		if (!@stat($fullpath) && !@stat($fullpathSGE))
		{
